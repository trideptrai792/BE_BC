...
{
    // Public: GET /api/flash-sale
    public function index(Request $request)
    {
        // so sánh theo timezone app
        ...
        $data = $items->map(function (FlashSale $item) {
            $p = $item->product; // có thể null, vẫn trả record

            return [
                'id'             => $item->id,
                'product_id'     => $item->product_id,
                'name'           => $p->name ?? null,
                'thumbnail'      => $p && $p->thumbnail ? asset($p->thumbnail) : null,
                'price'          => number_format($item->flash_price, 0, ',', '.') . 'đ',
                'old_price'      => ($p && $p->price_buy)
                    ? number_format($p->price_buy, 0, ',', '.') . 'đ'
                    : null,
                'sold'           => ($item->sold ?? 0) . ' đã bán',
                'discount_label' => $item->discount_label,
                'badge_left'     => $item->badge_left,
                'badge_right'    => $item->badge_right,
                'benefit_1'      => $item->benefit_1,
                'benefit_2'      => $item->benefit_2,
                'start_time'     => $item->start_at ? $item->start_at->toIso8601String() : null,
    ...
    // Admin: GET /api/admin/flash-sales
    ...
    // Admin: POST /api/admin/flash-sales
    ...
    // Admin: PUT /api/admin/flash-sales/{flashSale}
    ...
    // Admin: DELETE /api/admin/flash-sales/{flashSale}
